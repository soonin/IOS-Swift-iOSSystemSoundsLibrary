...
class ViewController: UIViewController , UITableViewDelegate, UITableViewDataSource {
    ...
    var modelList : [SysSoundsList] = []
    
    override func viewDidLoad() {
        super.viewDidLoad()
        
        for (soundkey,soundDes) in sysSoundsListArray {
            modelList.append(SysSoundsList(key: soundkey , description : soundDes))
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cellText = modelList[indexPath.row].key + " : " + modelList[indexPath.row].description
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let playSoundGroup = DispatchGroup()
        playSoundGroup.enter()
        updateText(dispatchGroup: playSoundGroup, index: indexPath.row)
        playSoundGroup.notify(queue: .main) {
            self.playSystemSound(soundID: UInt32(self.modelList[indexPath.row].key)!)
        }
    }

    func updateText(dispatchGroup : DispatchGroup , index : Int) {
        titleLabelView.text = modelList[index].key
        dispatchGroup.leave()
    }
    
    func playSystemSound(soundID: UInt32) {
        // create a sound ID, in this case its the tweet sound.
        let systemSoundID: SystemSoundID = soundID //1304 //1005 //1016
        // to play sound
